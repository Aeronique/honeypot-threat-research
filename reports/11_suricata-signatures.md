...
This report is part of a hands-on project focused on building practical skills in cyber threat intelligence. It is one report in a series covering a 28-day T-Pot honeypot deployment on Google Cloud Platform during February 2026. I'm open to constructive feedback.
...
Suricata is the intrusion detection system built into T-Pot. As traffic hits the honeypot, Suricata checks it against a library of known signatures and fires an alert when something matches.
...
Not every alert means an attacker was actively trying to exploit something. Some fire on reconnaissance traffic, others on scanning tools. Looking at which signatures fired most, and in what combination, is where the picture starts to form.
...
`ET INFO User-Agent (python-requests) Inbound to Webserver` fired **2,108,061** times. The `python-requests` library is the default HTTP client in many open-source scanning and exploitation frameworks. This volume points to large-scale automated scanning rather than manual activity.

### Scanning Tool Fingerprints

`ET SCAN MS Terminal Server Traffic on Non-standard Port` fired **242,279** times, indicating RDP-style traffic on ports other than 3389, a common enumeration technique.

`ET SCAN NMAP -sS window 1024` fired **27,194** times. Nmap's SYN scan sets a specific TCP window size that Suricata can detect directly. This is a clear fingerprint of Nmap activity.
...
`ET HUNTING RDP Authentication Bypass Attempt` fired **78,837** times. The `HUNTING` prefix indicates a signature for suspicious behavior not tied to a specific known exploit. RDP is a common ransomware delivery path, which makes this volume worth paying attention to.
...
`ET DROP Dshield Block Listed Source group 1` fired **88,036** times. This alert fires when traffic originates from an IP on the Dshield blocklist. The volume is consistent with findings in other reports in this series: a significant share of attacking IPs were already known to threat intelligence feeds before they ever hit this honeypot.
...
Alert counts reflect Suricata matching traffic against known patterns, not confirmed exploitation. The honeypot has no real services or credentials, so everything here represents attempts rather than successes.
